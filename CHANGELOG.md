...
Learnings from building **FootballCardPack** (a sealed foil pack with swipe-to-tear reveal, 3D card flip, staggered stat rings, and a full card browser) and **HeroPowerShowcase** (per-hero Canvas power effects: branching lightning, HUD rings, shockwave cracks, web mesh, orbiting infinity stones).

- **Swipe-to-reveal with masked image section (new)**: drive a `.mask(VStack { Rectangle().frame(height: h * progress); Spacer() })` from a drag gesture to progressively unveil a hidden image from top to bottom — reads as "peeling open" a sealed pack. Track both up *and* down: `max(0, -translation.height / 250)` so releasing the finger mid-reveal lets the image slide back smoothly instead of sticking. Phase gate: flip to `.sealed` when progress < 0.01 so the mask fully clears.
- **Counter-flip for cumulative `rotation3DEffect` (new)**: a Y-axis `rotation3DEffect` mirrors all content at odd multiples of 180°. Fix with `.scaleEffect(x: halfTurns % 2 != 0 ? -1 : 1)` on the inner content — compute `halfTurns = Int(round(angle / 180))` and the text/image always reads correctly regardless of cumulative rotations.
- **`matchedGeometryEffect` page dots (new, extends Page Indicator)**: instead of per-dot width animation (which snaps), render all dots as static circles and overlay one sliding `Capsule` with `.matchedGeometryEffect(id:in:)` — SwiftUI handles the interpolation perfectly, landing dead-center every time. Far smoother than manual offset math.
- **Staggered stat-ring fill with counting numbers (new)**: each `StatRingView` receives a `delay` parameter; on `.onAppear`, after `delay + 0.25s`, animate `.trim(from:to:)` with `.easeOut(duration: 1.2)` while a recursive `DispatchQueue` tick counts the displayed integer from 0 to target in sync. Use `.contentTransition(.numericText(countsDown: false))` on the number `Text` so each digit rolls up from below as it increments.
- **`PBXFileSystemSynchronizedRootGroup` auto-discovery (new, under Xcode Registration)**: projects with `objectVersion >= 77` use Xcode 16's file-system-synchronized groups — files placed in the source directory are compiled automatically with no `.pbxproj` edits needed. Check for `PBXFileSystemSynchronizedRootGroup` before attempting manual registration.
- **Type-checker timeout in large `ViewBuilder` bodies (extends State & Code Rules)**: closures with many chained modifiers, inline `let` computations, or `CGFloat.random()` calls in a `ForEach` can time out the Swift type-checker. Fix: extract into separate `View` structs (one body per concern), pre-compute random values into a static array via a plain function (never in a `@ViewBuilder`), and use `ForEach(Array(x.enumerated()), id: \.element.id)` instead of `ForEach(x.indices, id: \.self)` for stable identity.
- **Light/dark theme via `PackTheme` struct (new, extends Light Theme)**: a theme struct holding `bg`, `cardBg1/2`, `textPrimary/secondary`, `particleColor`, `borderOpacity`, and `scheme` — passed down to all subviews. Buttons adapt: dark mode uses glass-outline with spinning `AngularGradient` border; light mode uses solid 3D raised capsule with depth edge and border shimmer. Tier badges switch from tinted-text-on-subtle-bg (dark) to white-text-on-solid-color (light).
- **Per-hero Canvas power effects (new)**: use `TimelineView(.animation) + Canvas` for character-specific background animations. **Branching lightning**: a seeded `LightningTree` builds a bolt hierarchy with random forking; draw glow pass (blurred wide stroke) then hot-core pass (thin white) for depth; chromatic flash via blendMode `.plusLighter`. **HUD rings**: concentric arc segments rotating at different speeds + dashed strokes + tick marks + scan sweep + corner brackets + floating text readouts — all one Canvas. **Smash shockwave**: elliptical expanding rings + seeded crack paths growing from an impact origin + debris particles with gravity + screen shake via offset. **Web mesh**: radial spokes + concentric scalloped rings + corner quad-curve strands with glue droplets + hexagonal spider-sense rings. **Infinity stones orbit**: 6 colored dots on an elliptical path with comet trails (past-position sampling) + inter-stone energy links + periodic snap dust wave.
- **`SeededRNG` for deterministic Canvas particles (new, under State & Code Rules)**: a simple xorshift `struct SeededRNG` with `unit() -> CGFloat` avoids `CGFloat.random()` in the render loop, which would produce different positions every frame and flicker. The seed changes on events (new lightning strike, new smash) but stays constant frame-to-frame.
- Version output bumped to `swiftui-microinteractions v1.24.0`
